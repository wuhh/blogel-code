...
struct blk_less
{
	...
	{
		if(a->value().block<b->value().block) return true;
		else return false;
	}
};
...
class BAssignWorker{
	...
	public:
		...
		void load_vertex(BAssignVertex* v){//called by load_graph
		...
		virtual char* toline(BAssignVertex* v)=0;//this is what user specifies!!!!!!
