...
Marshall & operator<<(Marshall & m, const SPEdge & v){
	m<<v.len;
	m<<v.nb;
	m<<v.block;
	m<<v.worker;
...
Unmarshall & operator>>(Unmarshall & m, SPEdge & v){
	m>>v.len;
	m>>v.nb;
	m>>v.block;
	m>>v.worker;
...
Marshall & operator<<(Marshall & m, const SPValue & v){
	m<<v.dist;
	m<<v.from;
	m<<v.edges;
	return m;
...
Unmarshall & operator>>(Unmarshall & m, SPValue & v){
	m>>v.dist;
	m>>v.from;
	m>>v.edges;
	return m;
...
Marshall & operator<<(Marshall & m, const SPMsg & v){
	m<<v.dist;
	m<<v.from;
	return m;
}

Unmarshall & operator>>(Unmarshall & m, SPMsg & v){
	m>>v.dist;
	m>>v.from;
	return m;
}
...
class SPBlock:public Block<char, SPVertex, char>
{
	public:
		...
		{//heap is better than queue, since each vertex is enheaped only once
			...
			vector<elemT*> eles(n);
			for(int i=0; i<n; i++) eles[i]=NULL;//init eles[]
...
class SPBlockWorker:public BWorker<SPBlock>
{
	...
	public:
		virtual void blockInit(VertexContainer & vertexes, BlockContainer & blocks)
		{
			...
			//////
			if(_my_rank==MASTER_RANK) cout<<"Splitting in/out-block edges ..."<<endl;
			for(BlockIter it=blocks.begin(); it!=blocks.end(); it++)
			{
				...
				for(int i=block->begin; i<=block->end; i++)
				{
					...
					{
						if(edges[j].block==block->blockID)
						{
			...
			if(_my_rank==MASTER_RANK) cout<<"In/out-block edges split"<<endl;
		}
		...
		virtual void toline(SPBlock* b, SPVertex* v,BlockWriter* writer)
		{
            ...
            {
                sprintf(buf, "%d\t%d unreachable", v->id, b->blockID);
                writer->write(buf);
            }
		}
		//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
